...
class _EditPlaceScreenState extends State<EditPlaceScreen> {
  ...
  late List<vector.Vector3> _recordedWaypoints;
  ...
  void initState() {
    ...
    _selectedIcon = widget.place.icon;
    // Copy existing waypoints
    _recordedWaypoints = List.from(widget.place.waypoints);
  }
                    ...
                    children: [
                      Expanded(
                        child: OutlinedButton.icon(
                          ...
                          label: Text(
                            _isRecording ? 'Stop Rec' : 'Re-Record Path',
                          ),
  ...
  void _toggleRecording() {
    ...
    if (_isRecording) {
      // For Edit, if we start recording, we might want to clear existing waypoints
      // or append? Let's ask via dialog? Or just clear for simplicity as "Re-Record".
      // Let's clear to allow fresh path recording.
  ...
  void _savePlace() {
    ...
    // Preserve original ID and other fields, update editable ones
    // We update position to current location only if user "Recaptured" it?
    // For now, let's keep original position unless we want to allow moving the "Entrance".
    // But since we are recording path, the first point of path is usually the entrance.

    // If waypoints changed, update position to start of new path?
    vector.Vector3 newPosition = widget.place.position;
    if (_recordedWaypoints.isNotEmpty) {
      newPosition = _recordedWaypoints.first;
    }
    ...
    final updatedPlace = Place(
      ...
      icon: _selectedIcon,
      latitude: widget.place.latitude, // Keep original GPS for now
      longitude: widget.place.longitude,
      position: newPosition,
      waypoints: List.from(_recordedWaypoints),
      arImageUrl: widget.place.arImageUrl,
      distance: widget.place.distance,
      section: widget.place.section,
    );

    _controller.updatePlace(updatedPlace);
    Get.back();
